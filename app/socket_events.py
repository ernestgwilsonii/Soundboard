...
def on_join(data):
    ...
    print(f"DEBUG: on_join board_id={board_id}, user={current_user.username if current_user.is_authenticated else 'Anonymous'}")
    if not board_id:
        return
...
def on_request_lock(data):
    ...
    socketio.emit('slot_locked', {
        'sound_id': sound_id,
        'user': username
    }, to=str(board_id), include_self=False)
...
def on_release_lock(data):
    ...
    socketio.emit('slot_released', {'sound_id': sound_id}, to=str(board_id), include_self=False)
