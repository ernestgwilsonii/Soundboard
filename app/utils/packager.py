...
class Packager:
    ...
    def create_soundboard_pack(soundboard: Soundboard) -> io.BytesIO:
        ...
        with zipfile.ZipFile(memory_file, "w", zipfile.ZIP_DEFLATED) as zip_file:
            ...
            Packager._package_sounds(zip_file, soundboard, manifest)

            # 3. Package Board Icon
            Packager._package_board_icon(zip_file, soundboard, manifest)
    ...
    @staticmethod
    def _build_manifest_base(soundboard: Soundboard) -> Dict[str, Any]:
        ...
        return {
            ...
            "tags": [tag.name for tag in soundboard.get_tags()],
            "sounds": [],
        }

    @staticmethod
    def _package_sounds(
        zip_file: zipfile.ZipFile, soundboard: Soundboard, manifest: Dict[str, Any]
    ) -> None:
        ...
        for sound in soundboard.get_sounds():
            ...
            sound_data = Packager._package_single_sound(zip_file, sound)
            if sound_data:
                manifest["sounds"].append(sound_data)

    @staticmethod
    def _package_single_sound(
        zip_file: zipfile.ZipFile, sound: Sound
        ...
        zip_file.write(full_audio_path, arcname=f"sounds/{file_name}")
        ...
        if sound.icon and "/" in sound.icon:
            ...
            if os.path.exists(full_icon_path):
                ...
                zip_file.write(full_icon_path, arcname=zip_icon_path)
    ...
    def _package_board_icon(
        ...
        if os.path.exists(full_board_icon_path):
            zip_board_icon_path = f"icons/board_{os.path.basename(soundboard.icon)}"
            zip_file.write(full_board_icon_path, arcname=zip_board_icon_path)
