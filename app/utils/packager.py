...
class Packager:
    ...
    def create_soundboard_pack(soundboard):
        """
        Creates a ZIP archive containing the soundboard manifest and all associated assets.
        ...
        with zipfile.ZipFile(memory_file, 'w', zipfile.ZIP_DEFLATED) as zf:
            ...
            manifest = {
                ...
                'icon': soundboard.icon if not (soundboard.icon and '/' in soundboard.icon) else None,
            ...
            for sound in soundboard.get_sounds():
                ...
                full_audio_path = os.path.join(current_app.config['UPLOAD_FOLDER'], sound.file_path)
                if os.path.exists(full_audio_path):
                    zf.write(full_audio_path, arcname=f"sounds/{sound_data['file_name']}")
                    manifest['sounds'].append(sound_data)
                
                # Add custom sound icon if it's a file
                if sound.icon and '/' in sound.icon:
                    full_icon_path = os.path.join(current_app.config['UPLOAD_FOLDER'], sound.icon)
            ...
            if soundboard.icon and '/' in soundboard.icon:
                full_board_icon_path = os.path.join(current_app.config['UPLOAD_FOLDER'], soundboard.icon)
