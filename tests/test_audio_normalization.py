...
def test_normalize_success():
    ...
    with patch(
        "app.utils.audio.AudioSegment.from_file", return_value=mock_audio
    ) as mock_from_file:
        ...
        mock_normalized.export.assert_called_once_with(
            "dummy/path/song.mp3", format="mp3"
        )
...
def test_normalize_error_loading():
    ...
    with patch(
        "app.utils.audio.AudioSegment.from_file", side_effect=Exception("Load error")
    ):
...
@pytest.mark.parametrize(
    ...
    [("mp3", "mp3"), ("wav", "wav"), ("ogg", "ogg"), ("OGG", "ogg")],
)
def test_normalize_different_formats(extension, expected_format):
    ...
    with patch("app.utils.audio.AudioSegment.from_file", return_value=mock_audio):
        ...
        assert result is True
        mock_normalized.export.assert_called_once_with(
            file_path, format=expected_format
        )
