...
def test_explicit_ordering(app_context):
    with app_context.app_context():
        ...
        s1 = Sound(soundboard_id=sb.id, name='Later', file_path='1/1.mp3', display_order=10)
        s1.save()
        
        s2 = Sound(soundboard_id=sb.id, name='Earlier', file_path='1/2.mp3', display_order=5)
        s2.save()
...
def test_reorder_api(app_context):
    ...
    login_resp = client.post('/auth/login', data={'username': 'reorder', 'password': 'pass', 'submit': 'Sign In'}, follow_redirects=True)
