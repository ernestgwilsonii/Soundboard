...
def test_accounts_schema():
    ...
    assert "lockout_until" in columns
...
def test_admin_settings_schema():
    ...
    cursor.execute(
        "SELECT value FROM admin_settings WHERE key='featured_soundboard_id';"
    )
    assert cursor.fetchone() is not None
...
def test_soundboards_schema():
    ...
    cursor = db.cursor()
    cursor.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='soundboards';"
    )
    assert cursor.fetchone() is not None
...
def test_init_db():
    ...
    if os.path.exists(test_accounts_db):
        os.remove(test_accounts_db)
    if os.path.exists(test_soundboards_db):
