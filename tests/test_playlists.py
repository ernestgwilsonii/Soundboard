...
from app.models import User, Soundboard, Sound, Playlist
...
def test_playlist_crud(app_context):
    with app_context.app_context():
        ...
        # Read
        pl = Playlist.get_by_id(pl_id)
        assert pl.name == 'Chill Mix'
        
        # Update
        pl.name = 'Party Mix'
        pl.save()
        assert Playlist.get_by_id(pl_id).name == 'Party Mix'
...
def test_playlist_items(app_context):
    with app_context.app_context():
        ...
        assert len(pl.get_sounds()) == 1
        assert pl.get_sounds()[0].name == 'S2'
